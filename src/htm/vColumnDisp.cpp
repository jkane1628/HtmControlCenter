...
vColumnDisp::vColumnDisp(vView *_view, vDataSpace *_dataSpace, int _x, int _y)
{
   colX = _x;
   colY = _y;

	dpView = _view;
	dpDataSpace = _dataSpace;
	dpRegion = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_REGION) ? (vRegion*)dpDataSpace : NULL;
	dpInputSpace = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_INPUTSPACE) ? (vInputSpace*)dpDataSpace : NULL;
	
   numItems = dpDataSpace->GetSizeZ();
...
QRectF vColumnDisp::boundingRect() const
{
    return QRectF(-0.1, -0.1, 1.1, 1.1);
}

void vColumnDisp::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
    ...
    const qreal lod = option->levelOfDetailFromTransform(painter->worldTransform());
		...
		if (((dpView->GetViewConnectionsIn()) || (dpView->GetViewConnectionsOut())) && (sel_synapses.size() > 0))
		{
    ...
    painter->setBrush(fillColor);
		...
		int x = 0, y = 0;
      //vColumn column(dpRegion, colX, colY);
		...
		for (int i = 0; i < numItems; i++)
		{
         // If this cell is selected, first draw thicker black outline.
			...
			if (dpView->GetViewActivity())
			{
				// Determine whether the current cell is active.
            active = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_REGION) ? vCell(dpRegion, colX, colY, i).IsCellActive() : dpInputSpace->GetIsActive(colX, colY);

				// If not active, determine whether the current cell is predicted to be active in the future.
				if (!active) 
				{
               predicted = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_REGION) ? vCell(dpRegion, colX, colY, i).IsCellPredicted() : false;
               predicted_1step = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_REGION) ? vCell(dpRegion, colX, colY, i).GetNumPredictionSteps()==1 : false;
            ...
            learning = (dpDataSpace->GetDataSpaceType() == VDATASPACE_TYPE_REGION) ? vCell(dpRegion, colX, colY, i).IsCellLearning() : false;
			...
			else if (dpView->GetViewReconstruction() || dpView->GetViewPrediction())
			{
			...
			if (dpView->GetViewActivity() && (lod >= 50) && (predicted || learning))
			{
				...
				else 
				{
					...
					str.setNum(vCell(dpRegion, colX, colY, i).GetNumPredictionSteps());
				}
			...
			if (dpView->GetViewMarkedCells())
			{
				int index = (colY * dpView->sceneWidth * numItems) + (colX * numItems) + i;
				std::map<int,int>::iterator iter = dpView->marked_cells.find(index);
				if (iter != dpView->marked_cells.end())
				{
...
int vColumnDisp::DetermineCellIndex(float _x, float _y)
{
	int x = 0, y = 0;
	QRectF rect;
	for (int i = 0; i < numItems; i++)
...
QColor vColumnDisp::DetermineSynapseColor(float _permanence, float _connectedPerm)
{
	float degree;
...
void vColumnDisp::SetSelection(bool _colSelected, int _selCellIndex)
{
...
void vColumnDisp::SelectSynapse(int _cellIndex, float permanence, float connectedPermanence)
{
	if ((sel_synapses[_cellIndex].permanence < permanence)) {
		sel_synapses[_cellIndex] = SynapseRecord(_cellIndex, permanence, connectedPermanence);
	}
...
void vColumnDisp::ClearSelectedSynapses()
{
	sel_synapses.clear();
...
void vColumnDisp::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsItem::mousePressEvent(event);
    update();
}

void vColumnDisp::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
...
void vColumnDisp::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
